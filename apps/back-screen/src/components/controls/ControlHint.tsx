...
export function ControlHint({ control, side = "left" }: ControlHintProps) {
  ...
  const marker = isCabinet ? (
    <span
      className={`flex size-6.5 items-center justify-center rounded-full font-mono text-[clamp(0.36rem,0.8vw,0.56rem)] font-bold tracking-wider ${control.cabinet.filled ? "border border-white/70 bg-white/85 text-[#0a0d14] shadow-[0_0_10px_rgba(255,255,255,0.35)]" : "border border-white/45 bg-transparent text-white/70"}`}
    >
  ...
  const action = (
    <span
      className={`${isCabinet ? "text-[clamp(0.5rem,1.05vw,0.78rem)] text-[rgba(243,230,0,0.6)]" : "text-[clamp(0.34rem,0.7vw,0.5rem)] text-[rgba(243,230,0,0.55)]"} font-mono tracking-[0.2em] uppercase`}
    >
      {control.action}
    </span>
  )
