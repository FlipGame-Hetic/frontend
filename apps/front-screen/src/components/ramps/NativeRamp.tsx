...
interface NativeRampProps {
  points: RampPathPoints
  settings: NativeRampSettings
}
...
export default function NativeRamp({ points, settings }: NativeRampProps) {
  const curve = useMemo(
    () => new THREE.CatmullRomCurve3(points, false, "catmullrom", 0.5),
    [points],
  )
  const segments = useMemo(() => buildNativeSegments(curve, settings), [curve, settings])
  const activeBallsRef = useRef<Map<string, ActiveNativeBall>>(new Map())
  ...
  return (
    <group>
      {segments.map((segment) => (
        <Fragment key={`native-ramp-visual-${String(segment.id)}`}>
          <mesh
      ...
      <RigidBody type="fixed" colliders={false}>
        {segments.map((segment) => (
          <Fragment key={`native-ramp-collider-${String(segment.id)}`}>
            <CuboidCollider
              args={[
